...
class MBConvBlock(nn.Module):
    ...
    def __init__(self, block_args, global_params, ktype):
        ...
        if self.k == 5:
            ...
            self._bn1_k = nn.BatchNorm2d(num_features=oup, momentum=self._bn_mom, eps=self._bn_eps)
        if self.k == 7:
            self._depthwise_conv_k_2 = Conv2d(
                in_channels=oup, out_channels=oup, groups=oup,  # groups makes it depthwise
                kernel_size=3, stride=1, bias=False)
            self._bn1_k_2 = nn.BatchNorm2d(num_features=oup, momentum=self._bn_mom, eps=self._bn_eps)
    ...
    def forward(self, inputs, drop_connect_rate=None):
        ...
        if self.k == 5:
            x = self._swish(self._bn1_k(self._depthwise_conv_k(x)))
        if self.k == 7:
            x = self._swish(self._bn1_k_2(self._depthwise_conv_k_2(x)))
...
class EfficientNet(nn.Module):
    ...
    def __init__(self, blocks_args=None, global_params=None, first_stride=True, ktype=False):
        ...
        s = 2 if first_stride else 1
        self._conv_stem = Conv2d(in_channels, out_channels, kernel_size=3, stride=s, bias=False)
